...
def auto_corr(x1, x2, x3, x4, Y):
    ...
    est_thetta = f.parameter_estimation_tetta(matr_X, Y)
    DW, fl = test_Durbin_Watson(matr_X, est_thetta, Y, N)
    est_t = Cochrane_Oreutt_procedure(matr_X,  Y, N)
    ##############################################3
def test_Durbin_Watson(matr_X, est_thetta, Y, N):
    est_e = Y - np.matmul(matr_X, est_thetta)
    DW2 = np.sum(est_e ** 2)
...
def search_est_p(matr_X, Y, est_e):
    ...
    d_0 = np.sum(tmp)
    est_p1 = d_0 / d
    est_thetta1 = parameter_estimation_OMNK(est_p1, matr_X, Y)
    est_e1 = Y - np.matmul(matr_X, est_thetta1) 
    return est_p1, est_thetta1, est_e1
############################################
def Cochrane_Oreutt_procedure(matr_X,  Y, N):
    est_thetta = f.parameter_estimation_tetta(matr_X, Y)
    est_e = Y - np.matmul(matr_X, est_thetta)
    est_p1, est_thetta1, est_e1 = search_est_p(matr_X, Y, est_e)
    est_p2, est_thetta2, est_e2 = search_est_p(matr_X, Y, est_e1)
    est_p3, est_thetta3, est_e3 = search_est_p(matr_X, Y, est_e2)
    est_p4, est_thetta4, est_e4 = search_est_p(matr_X, Y, est_e3)
    est_p5, est_thetta5, est_e5 = search_est_p(matr_X, Y, est_e4)
    return est_thetta4